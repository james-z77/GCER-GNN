...
class data_process(object):
        ...
        def g_graph(text):
            ...
            for i in text.keys():
                ...
                for review in reviews:
                    ...
                    subG = nx.Graph()
                    subG.add_weighted_edges_from(edges_list)
                    if len(subG.nodes) == 0 or len(subG.edges) == 0:
                        continue
                    node_list = np.array([x[0] for x in c.most_common(num_node)])
                    sub = nx.to_scipy_sparse_matrix(subG, nodelist=node_list)
                    index_val = sp.find(sub)
                    edg_index = np.array([index_val[1], index_val[0]])
                    edg_value = np.array(index_val[2])
                    sub_graph.append([edg_index, edg_value, node_list])
    ...
    def process_d(self):
        ...
        self.user_num = len(self.u_text)  # tokenize
